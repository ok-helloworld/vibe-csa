...
> 当项目语言**没有**对应的 `plugins/<lang>/` 时（Node/TypeScript/Ruby/Rust/C#/Kotlin 非 JVM 项目等），核心协议自动加载本插件，以降级模式继续审计。
...
| PHP | `plugins/php/` |
| Go | `plugins/go/` |
| **其他**（Node / TypeScript / C# / Ruby / Rust / C/C++ / Scala 等） | `plugins/_generic/` |
...
2. 多 Agent 各自记录“我能识别什么不能识别”——例如 Agent-Injection 不熟悉 Rust 的 ORM，应明示，由后续汇总与验证阶段复核
3. 框架识别失败时直接 grep `route` / `handler` / `controller` / `mux` / `app.get` / `app.post` 等关键字找入口
4. 多 Agent 报告中标注 `x_plugin_mode = "generic"`，便于审计后定位降级影响
5. 对 `sinks.md` 中“资源加载 / 动态消费补强”和“配置驱动加载与二次消费候选点”命中项，不得停在 grep 命中；必须继续追踪配置值、环境变量、数据库值是否进入模板、模块、插件、资源或脚本加载点，并优先判断是否可与文件上传或文件写入组链
...
```json
{
  "audit": {
    ...
    "x_generic_reason": "no plugins/typescript/ available; static analysis quality degraded"
  }
}
```
...
1. **Node.js / TypeScript**：Web 后端体量最大，框架（Express/Koa/NestJS/Next.js）复杂但样式集中
2. **C# / .NET**：企业项目高频，ASP.NET Core 路由+Razor+EF Core 主导
3. **Ruby**：Rails 主导，单一框架反而好做
4. **Rust**：actix-web / axum / rocket，量小但增长快
5. **C/C++**：需要单独项目，Sink 分析方式差异大（栈溢出/整数溢出/Use-After-Free 等）
