...
对“配置驱动模板加载 / 动态导入 / 资源加载 / 上传后二次消费”候选点，不得停在 grep 命中；必须继续追踪配置值、数据库值、环境变量是否进入模板名、模块名、资源路径等消费点，并优先判断是否可与文件上传或文件写入组链。
...
- [ ] 配置值进入模板名、模块名、资源路径或动态导入消费点
- [ ] 上传或文件写入结果可被模板、动态导入、插件、实例目录机制二次消费
- [ ] 文件上传无类型/大小校验
