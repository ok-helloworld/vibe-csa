...
PoC 验证结果必须基于**响应体中可直接证明漏洞存在的具体内容**，且**由 verify_vuln.py 自动签名匹配判定**——LLM 不再被信任独立判定漏洞是否成立：
...
**强制执行（脚本层）**：
- `verify_vuln.py` 按 finding 的 `x_signature_type`（参照 `references/exploit-success-signatures.md`）自动跑正则匹配
- 命中结果写入 `poc.steps[i].response._evidence_match`，**LLM 不可手填或编造**
- `_consistency.py` 在签名相关漏洞类型中：`poc.result=success` 但所有 step 均无 `_evidence_match` → **BLOCKING error**
